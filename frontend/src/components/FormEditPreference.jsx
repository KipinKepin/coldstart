...
const FormEditPreference = () => {
    ...
    useEffect(() => {
        const getPreferenceById = async () => {
            try {
                const response = await axios.get(`http://localhost:5000/preferences/${id}`)
                setGender(response.data.gender)
                setAge(response.data.age)
                setFitness(response.data.fitnessLevel)
                setFrequency(response.data.exerciseFrequency)
                setDuration(response.data.exerciseDuration)
                setDiet(response.data.balanceDiet)
    ...
    const updatePreference = async (e) => {
        ...
        try {
            await axios.patch(`http://localhost:5000/preferences/${id}`, {
                gender: gender,
                age: age,
    ...
    return (
        <div className="add">
            <em className="subtitle has-text-dark" style={{ fontSize: '1rem' }}>Edit preference</em>
...
export default FormEditPreference;
